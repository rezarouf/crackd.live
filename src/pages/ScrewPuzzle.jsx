...
export default function ScrewPuzzlePage({ setPage }) {
  const [screws] = useState(INITIAL_SCREWS);
  ...
  const handleScrew = (screw) => {
    ...
    if (selected && selected.id !== screw.id) {
      if (selected.color === screw.color) {
        const newFreed = [...freed, selected.id, screw.id];
        setFreed(newFreed);
        setSelected(null);
        setMoves(m => m + 1);
        if (newFreed.length >= INITIAL_SCREWS.length) setTimeout(() => setShowWin(true), 500);
      } else {
        setSelected(screw);
        setMoves(m => m + 1);
      }
  ...
  return (
    <div style={{ background: '#0D0F14', minHeight: '100vh', color: '#F0F0F0', fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 80 }}>
      ...
      <div style={{ width: '100%', maxWidth: 600, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 0', borderBottom: '1px solid rgba(255,255,255,0.06)', marginBottom: 32 }}>
        <button onClick={() => setPage('games')} style={{ background: 'none', border: 'none', color: '#8B95A1', cursor: 'pointer', fontSize: 14, fontFamily: 'Inter' }}>← Back</button>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 18, fontWeight: 800 }}>Screw Puzzle</span>
          <span style={{ fontSize: 12, fontWeight: 700, color: DIFF_COLOR['Easy'], background: `${DIFF_COLOR['Easy']}15`, padding: '3px 10px', borderRadius: 20 }}>Easy</span>
        </div>
                    ...
                    <div key={col} style={{ width: 52, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {screw ? (
                        <div onClick={() => handleScrew(screw)} style={{
                          ...
                          {!isFree && <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'rgba(0,0,0,0.4)', boxShadow: 'inset 0 1px 2px rgba(255,255,255,0.2)' }} />}
